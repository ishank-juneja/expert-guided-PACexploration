...
from Advice_class import Advice
from Utils import *
import argparse
import pathlib
import sys
...
def main(cmd_args, fout):
    ...
    if cmd_args.al is not None:
        ...
        algos = ['eps-greedy', 'action-elimination']

    # Initialise MDP sample model instance by parsing text file with true MDP parameters
    ...
    # Get optimal policy to use as external expert
    v_opt, pi_optimal = mdp.plan()
    ...
    for al in algos:
        for rs in range(cmd_args.nseeds):
            ...
            if al == 'eps-greedy':
                ...
                advice = Advice(model, pi_optimal)
